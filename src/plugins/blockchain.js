...
const DNA = Exonum.newType({
  fields: [{ name: "dna", type: Exonum.Uint32 }]
});
...
function getSystemTime() {
  ...
  const nanos = bigInt(now)
    .minus(secs.multiply(1000))
    .multiply(1000000);
...
function waitForAcceptance(response) {
  ...
  return (function makeAttempt() {
    return axios
      .get(`/api/explorer/v1/transactions/${response.data.tx_hash}`)
      .then(response => {
  ...
  install(Vue) {
    Vue.prototype.$blockchain = {
      createUser: (name, role) => {
        ...
        // Transaction data
        const data = { public_key: keyPair.publicKey, name: name, role: role };
        ...
        return axios
          .post("/api/services/cryptoowls/v1/transaction", {
            network_id: NETWORK_ID,
            protocol_version: PROTOCOL_VERSION,
            service_id: SERVICE_ID,
            message_id: CREATE_USER_TX_ID,
      ...
      getUsers: () => {
        return axios
          .get("/api/services/cryptoowls/v1/users")
      ...
      getUserOrders: publicKey => {
        return axios
          .get(`/api/services/cryptoowls/v1/user/${publicKey}/orders`)
          .then(response => response.data);
      },

      getOwls: () => {
        return axios
          .get("/api/services/cryptoowls/v1/owls")
          .then(response => response.data);
      },

      getUserOwls: publicKey => {
        return axios
          .get(`/api/services/cryptoowls/v1/user/${publicKey}/owls`)
          .then(response => response.data);
      },

      getOwl: hash => {
        return axios
          .get(`/api/services/cryptoowls/v1/owl/${hash}`)
          .then(response => response.data);
      },

      getOrders: hash => {
        return axios
          .get(`/api/services/cryptoowls/v1/owl/${hash}/orders`)
          .then(response => response.data);
      },
      ...
      getBlocks: latest => {
        ...
        return axios
          .get(`/api/explorer/v1/blocks?count=10${suffix}`)
          .then(response => response.data.blocks);
      },

      getBlock: height => {
        return axios
          .get(`/api/explorer/v1/blocks/${height}`)
          .then(response => response.data);
      },

      getTransaction: hash => {
        return axios
          .get(`/api/explorer/v1/transactions/${hash}`)
          .then(response => response.data);
      },
      ...
      splitDNA: dna => {
        ...
        const color = Exonum.uint8ArrayToHexadecimal(
          new Uint8Array(buffer.slice(0, 3))
        );

        // Convert fourth element into binary string
        const appearance = Exonum.uint8ArrayToBinaryString(
